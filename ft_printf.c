...
/*   Updated: 2024/02/21 18:05:37 by dfontech         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{ // donde: FORMATO es una cadena qque contiene un texto que se escribir´en la salid estándar.  ARGUMENTOS ADICIONALES (tres punts se llaman elipses) que indica el número variable de argumentos dependiendo del formato de la cadena//
	...
	while (cadena[i])
	{
		...
		{
			i++;
			length += ft_format(va, &cadena[i]); //& es un operador de referencia, se utiliza para almacenar la dirección de memoria de la variable y la asigna al puntero
		}
		else
			length += ft_putchar(cadena[i]); // imprimir el caracter recibido e incrementar el valor apuntaddo por counter para mostrar el número toal de caracteres impresos
		i++;
	}
